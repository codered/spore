...
Its working directory is the policy ceiling, not the calling session's
workspace: one MCP server process is shared by every session, so it cannot
hold a per-session directory. Containment for MCP calls comes from the policy
engine judging path arguments against the calling session's workspace.
