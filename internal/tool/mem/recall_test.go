...
func localCtx(sid string) context.Context {
	return policy.WithSession(context.Background(), policy.Session{ID: sid, Profile: policy.ProfileLocal})
}
...
func TestRecallSearchRemoteProfileIsScoped(t *testing.T) {
	f := &fakeRecall{}
	ctx := policy.WithSession(context.Background(), policy.Session{ID: "remote-sess", Profile: policy.ProfileRemote})
...
func TestRecallSearchUnknownProfileIsScopedLikeRemote(t *testing.T) {
	f := &fakeRecall{}
	ctx := policy.WithSession(context.Background(), policy.Session{ID: "guest-sess", Profile: policy.Profile("guest")})
	if _, err := NewRecallSearch(f).Call(ctx, json.RawMessage(`{"query":"x"}`)); err != nil {
		t.Fatal(err)
	}
