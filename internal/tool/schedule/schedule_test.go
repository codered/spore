...
func TestDefaultPolicyGating(t *testing.T) {
	...
	result := engine.Evaluate(policy.Session{Profile: policy.ProfileLocal}, policy.Call{Tool: "schedule_list", Args: json.RawMessage(`{}`)})
	...
	for _, name := range []string{"schedule_create", "schedule_cancel"} {
		result := engine.Evaluate(policy.Session{Profile: policy.ProfileLocal}, policy.Call{Tool: name, Args: json.RawMessage(`{}`)})
