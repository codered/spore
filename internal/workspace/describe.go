...
const (
	...
	// every one of them on every request. Only a flat root with that many
	// entries can reach it, since the listing never descends.
	maxEntries = 200
...
// noiseDirs are never listed. .git is excluded because git
...
// list returns the entries of root itself, directories marked with a
// trailing slash. It does not descend: what a subdirectory holds is one
// fs_list call away, and a recursive walk put hundreds of names into every
// request for a model that usually needed none of them.
//
// The second result reports whether the entry budget cut the listing short.
// The third reports whether a .gitignore was read, so the caller can describe
// the listing honestly.
func list(root string) ([]string, bool, bool) {
	rules, filtered := ignoreStack(nil).load(root, "")
	entries, err := os.ReadDir(root)
	if err != nil {
		return nil, false, filtered
	}
